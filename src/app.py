...
df_gdp["GDP per capita (current US$)"] = df_gdp["GDP per capita (current US$)"].astype(
    float
)
        ...
        html.Div(
            [
                ...
                dcc.Dropdown(
                    ...
                    options=[
                        {"value": x, "label": x}
                        for x in df_ghg_country["year"].unique()
                    ],
                ),
            ],
            className="mb-4",
        ...
        html.Div(
            [
                ...
                dcc.Checklist(
                    ...
                    options=[
                        {"value": x, "label": x}
                        for x in df_ghg_country["continent"].unique()
                    ],
                ),
            ],
            className="mb-4",
...
def update_plot(year_, continent):
    ...
    if continent is None:
        ...
        mask_continent = df_ghg_total["continent"].isin(
            continent
